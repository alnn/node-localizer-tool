...
function fetchLocaleTerms(section, locale) {
  return {
    ...
    requestingAPI: function requestingAPI() {
      return Term.getList(section.id, locale.id).then(function (result) {
...
function updateLocaleTerm(id, phrase) {
  return {
    ...
    requestingAPI: function requestingAPI() {
      return Term.findOneAndUpdate({ _id: id }, { phrase: phrase }).exec().then(function (term) {
...
function deleteLocaleTerm(id) {
  return {
    ...
    requestingAPI: function requestingAPI() {
      return Term.removeById(id).then(function () {
        return id;
      });
    }
  };
